...
gameId = sys.argv[1]
print(gameId)
Api=MakeMove('MontyPython',int(gameId),665337)
...
while True:
	...
	if can_steal_koalas(go,my_pos,opp_pos,player,d) and d['player'+other]['gatheredKoalas']>1:
		print(d['player1']['x'],d['player1']['y'])
		print(d['player2']['x'],d['player2']['y'])
		d,matrix=Api.StealKoalas()
	elif can_steal_koalas(go,my_pos,opp_pos,player,d):
		invalid_move=0
		for i in range(6):
				...
				for i in range(6):
					...
					if dest==None or (not check_cell(dest,matrix)):
						...
						for j in range(6):
							...
							if nxt!=None and check_cell(nxt,matrix) and dist[nxt[0]][nxt[1]]!=1:
								good=True
